...
export function createDemoFlow(): FlowGraph {
  const specs: Array<[string, FlowNodeType, string, number, number, Record<string, unknown>?]> = [
    ...
    ['randomKeeper', 'RandomNode', 'Generate Keeper Window', 1480, 40, { mode: 'number_between', min: 2, max: 15, outputVariable: 'keeperWindowMinutes' }],
    ['waitKeeper', 'TimerNode', 'Wait Keeper Window', 1770, 40, { mode: 'wait_for_duration', durationSource: 'variable', durationVariable: 'keeperWindowMinutes', unit: 'minutes' }],
    ['store', 'ActionNode', 'Store Shuffle State', 2060, 40, { actionName: 'storeShuffleState' }],
    ['countWaiting', 'DbQueryNode', 'Count Waiting Rooms', 2350, 130, { queryName: 'countWaitingRooms', table: 'rooms', operation: 'count', filters: '[{"status":"waiting"}]', outputVariable: 'waitingRooms' }],
    ['belowCap', 'ConditionNode', 'waitingRooms < roomCountShuffle?', 2640, 130, { left: 'waitingRooms', operator: 'less_than', right: 'roomCountShuffle', trueLabel: 'true', falseLabel: 'false' }],
    ['stop2', 'EndNode', 'Stop: Capacity Reached', 2920, 330, { resultStatus: 'stopped' }],
    ['exists', 'ConditionNode', 'Existing Waiting Room?', 2930, 70, { left: 'waitingRooms', operator: 'greater_than', right: '0', trueLabel: 'yes', falseLabel: 'no' }],
    ['create', 'ActionNode', 'Create Waiting Room', 3220, -20, { actionName: 'createWaitingRoom' }],
    ['countPlayers', 'DbQueryNode', 'Count Normal Players', 3220, 150, { queryName: 'countNormalPlayers', table: 'players', operation: 'count', filters: '[{"kind":"normal"}]', outputVariable: 'normalPlayers' }],
    ['enough', 'ConditionNode', 'normalPlayers >= minNormalPlayers?', 3510, 150, { left: 'normalPlayers', operator: 'greater_or_equal', right: 'minNormalPlayers', trueLabel: 'true', falseLabel: 'false' }],
    ['createAdditional', 'ActionNode', 'Create Additional Waiting Room', 3800, 50, { actionName: 'createWaitingRoom' }],
    ['stop3', 'EndNode', 'Stop: Not Enough Players', 3800, 250, { resultStatus: 'stopped' }],
    ['success', 'EndNode', 'End Success', 4100, 20, { resultStatus: 'success' }],
  ]
  ...
  const edges = [
    ...
    link('randomCap', 'randomKeeper'), link('randomKeeper', 'waitKeeper'), link('waitKeeper', 'store'), link('store', 'countWaiting'),
  ...
  return {
    ...
    variables: [
      ...
      { id: 'var_keeper_minutes', name: 'keeperWindowMinutes', type: 'number', defaultValue: 2, description: 'Minutes before a new shuffle is generated.', scope: 'nodeOutput' },
