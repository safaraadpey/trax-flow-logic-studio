...
function bingoSchedulerFlow(prompt: string): FlowGraph {
  ...
  const nodes = [
    ...
    semanticNode('ai_count_waiting', 'DbQueryNode', 'Count Waiting Rooms', 'Models a count query for rooms currently waiting for players.', 1420, 100, { queryName: 'countWaitingRooms', table: 'rooms', operation: 'count', filters: '[{"status":"waiting","templateId":"context.templateId"}]', outputVariable: 'waitingRooms' }),
  ...
  const edges = [
    ...
    edge('ai_random_keeper', 'ai_count_waiting'),
  ...
  const variables: FlowVariable[] = [
    { name: 'roomCountShuffle', type: 'number', defaultValue: 0, description: 'Randomized waiting-room capacity.' },
    { name: 'keeperWindowMinutes', type: 'number', defaultValue: 2, description: 'Minutes for which the randomized cap is retained.' },
    { name: 'waitingRooms', type: 'number', defaultValue: 0, description: 'Current number of waiting rooms.' },
    { name: 'waitingRoom', type: 'object', defaultValue: null, description: 'Existing waiting room, when found.' },
    { name: 'normalPlayers', type: 'number', defaultValue: 0, description: 'Normal-player count used for capacity decisions.' },
  ]
...
function simpleFlow(prompt: string): FlowGraph {
  ...
  return {
    ...
    variables: [{ name: 'result', type: 'object', defaultValue: null, description: 'Result produced by the generated action.' }],
